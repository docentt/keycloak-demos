...
if [ -z ""${KC##*keycloak-demos*} ]; then
  ...
  docker run --name=keycloak-demos-dump -v $(pwd)/DUMPS/$currentTime:/opt/keycloak/DUMP --entrypoint /opt/keycloak/bin/kc.sh keycloak-demos-dump export --dir /opt/keycloak/DUMP
	docker rm keycloak-demos-dump
	docker rmi keycloak-demos-dump
else
